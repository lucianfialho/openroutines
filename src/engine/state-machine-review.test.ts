/**
 * Engine-level flow tests for the `review`/`refutation` states (F4 #153) —
 * mirrors the transition shapes in .gates/skills/card-to-pr/skill.yaml with a
 * synthetic skill + mocked providers (the REAL aggregateReview wired in),
...
/** Mirrors .gates/skills/card-to-pr/skill.yaml's verify->review->refutation wiring. */
const cardSkill = (): SkillStateMachine =>
  ({
    id: "t",
    initial_state: "verify",
    states: {
      verify: { type: "script", script: "verify", transitions: [{ to: "review" }] },
      review: {
        type: "fanout",
        lenses: [
        ...
        transitions: [
          { to: "refutation", when: "output.review.gaps.length > 0", max_retries: 2, on_exhausted: "blocked" },
          { to: "blocked", when: "output.review.securityVerdict.approved == false" },
          { to: "pr", when: "output.review.approved == true" },
          { to: "blocked" },
        ],
      },
      refutation: {
        agent_prompt: "responda aos gaps: {{outputs.review.gaps}}",
        transitions: [
          { to: "implementation", when: "output.refutation.status == 'corrigir'" },
          { to: "review", when: "output.refutation.status == 'contestado'" },
        ],
      },
      implementation: { terminal: true },
      pr: { terminal: true },
...
describe("review/refutation flow (#153)", () => {
  it("dataChanges:false — the data lens is never invoked and an all-approved review lands on pr", async () => {
    ...
    const r = await run(cardSkill(), {
      ...
      fanoutAggregators: { aggregateReview },
    });
  ...
  it("a contestable correctness gap routes to refutation; {status:'corrigir'} routes on to implementation", async () => {
    ...
    expect(r.success).toBe(true);
    expect(r.logs.join(" ")).toContain("Reached terminal state: implementation");
  });

  it("{status:'contestado'} routes back to review, and the re-invoked lens's prompt carries the evidence", async () => {
    ...
    (skill.states.review.lenses![0] as Record<string, unknown>).agent_prompt =
      "review correctness <contestacao_refutacao>{{outputs.refutation}}</contestacao_refutacao>";
    const r = await run(skill, {
      provider: {
        complete: () => Effect.succeed(resp({ content: JSON.stringify({ status: "contestado", evidencia: "o plan aprovou isso" }) })),
      },
      providerRegistry: registry,
      scriptRegistry: registerVerify(false),
      repository: makeRepo().repo,
    ...
    expect(kimiCalls[1].request.messages?.[0]?.content).toContain("o plan aprovou isso");
  });

  it("caps review->refutation at 2 retries, then escapes to blocked (on_exhausted) instead of failing the execution", async () => {
    const { registry } = makeTrackingRegistry({
      // correctness ALWAYS reports the same contestable gap; refutation ALWAYS contests it — a stuck adjudication loop.
      "kimi-cli:kimi-k2.6": JSON.stringify({ approved: false, gaps: [{ description: "missing AC", contestable: true }] }),
      "security-judge:claude-opus-4-8": approvedSecurity,
    });
    const r = await run(cardSkill(), {
    ...
    expect(r.success).toBe(true); // on_exhausted escapes to blocked; it does not fail the execution
    expect(r.logs.join(" ")).toContain("Reached terminal state: blocked");
  });

  it("an 'open' security finding routes to refutation, NOT directly to blocked (explicit transition-order check)", async () => {
    const { registry } = makeTrackingRegistry({
      "kimi-cli:kimi-k2.6": approvedCorrectness,
    ...
    expect(r.success).toBe(true);
    expect(r.logs.join(" ")).toContain("Reached terminal state: implementation");
    expect(r.logs.join(" ")).not.toContain("Reached terminal state: blocked");
  });

  it("a terminal reproved security verdict (no open findings left) routes straight to blocked", async () => {
    const { registry } = makeTrackingRegistry({
      "kimi-cli:kimi-k2.6": approvedCorrectness,
      "security-judge:claude-opus-4-8": JSON.stringify({
        approved: false,
