...
import { makeDelivery, formatProposalMarkdown, type DeliveryOutput } from "./delivery.js";
...
const doc = (phaseCount: number): SurveyDoc => ({
...
const baseDeps = (rec: Record_): ResearchDeps => ({
  registry: { repos: {} },
  githubToken: "gh",
  worktreeBase: "/tmp",
  claudeApiKey: "sk",
...
const run = (deps: ResearchDeps, outputs: Record<string, unknown>) =>
  makeDelivery(deps)({ inputs, outputs, executionId: "e", stateId: "delivery" }) as Promise<DeliveryOutput>;

describe("card-research delivery", () => {
  it("phases=4 -> a milestone with 4 issues, each linked to it by number (criterion 4)", async () => {
    const rec = freshRecord();
    const out = await run(baseDeps(rec), { preparation: prep, survey_proposal: doc(4), architecture_judgment: parecer });
  ...
  it("phases=2 -> 2 loose issues, NO milestone (criterion 4)", async () => {
    const rec = freshRecord();
    const out = await run(baseDeps(rec), { preparation: prep, survey_proposal: doc(2), architecture_judgment: parecer });
  ...
  it("phases=3 -> milestone (the >= 3 boundary is inclusive)", async () => {
    const rec = freshRecord();
    await run(baseDeps(rec), { preparation: prep, survey_proposal: doc(3), architecture_judgment: parecer });
    expect(rec.milestones).toHaveLength(1);
  ...
  it("degrades to card-only delivery when no repo resolved (no GitHub issue, still delivered)", async () => {
    const rec = freshRecord();
    const out = await run(baseDeps(rec), { preparation: { repos: [] }, survey_proposal: doc(2), architecture_judgment: parecer });
  ...
  it("idempotent delivery (F5 #162): a second run on a 'done' ledger never re-creates the issue", async () => {
    ...
    const deps: ResearchDeps = { ...baseDeps(rec), ledger };
    const outputs = { preparation: prep, survey_proposal: doc(2), architecture_judgment: parecer };
