...
const baseDeps = (o: Partial<ResearchDeps> = {}): ResearchDeps => ({
  registry,
  githubToken: "gh",
...
const ctx = (over: Record<string, unknown> = {}) => ({ inputs, outputs: {}, executionId: "e", stateId: "preparation", ...over });

describe("card-research preparation", () => {
  it("resolves the repo and mounts a DETACHED read-only worktree at the base branch", async () => {
    ...
    const r = (await makePreparation(baseDeps({ runGit, worktreeBase: wtBase }))(ctx())) as PreparationOutput;
  ...
  it("is tolerant: an unresolvable repo yields no repos, no worktree, and no git call", async () => {
    const runGit = vi.fn(async () => ({ stdout: "", stderr: "" }));
    const r = (await makePreparation(baseDeps({ runGit }))(ctx({ inputs: { ...inputs, repo: "does-not-exist" } }))) as PreparationOutput;
  ...
  it("resolves multiple repo fields but only worktrees the first", async () => {
    ...
    const runGit = vi.fn(async () => ({ stdout: "", stderr: "" }));
    const r = (await makePreparation(baseDeps({ registry: reg, runGit }))(
      ctx({ inputs: { ...inputs, repo: "acme-widgets, acme-api" } })
    )) as PreparationOutput;
  ...
  it("reads docs/REPO-PROFILE.md into the repo context (best-effort)", async () => {
    ...
    try {
      ...
      const runGit = vi.fn(async () => ({ stdout: "", stderr: "" }));
      const r = (await makePreparation(baseDeps({ registry: reg, runGit, worktreeBase: `/tmp/or-pesquisa-wt-${Date.now()}` }))(ctx())) as PreparationOutput;
