/**
 * card-research / preparation (F5 #161)
 *
 ...
 * shift it mid-survey).
 *
 * "read-only" is enforced by the survey phase's tool allowlist (no
 * Write/Edit), NOT by filesystem permissions — the detached worktree exists
 * only to pin the view. Unlike card-to-pr's preparation there is no branch
...
import { defaultRunGit, type ResearchDeps } from "./index.js";

export interface PreparationRepo {
  slug: string;
  githubRepo: string;
  clonePath: string;
  baseBranch: string;
...
export interface PreparationOutput {
  repos: PreparationRepo[];
  worktree?: { path: string };
}

// RepoConfig carries no key of its own (repos.yaml maps key -> config) — recover
// the "slug" by reference, same as card-to-pr's preparation does.
...
export const makePreparation = (deps: ResearchDeps): ScriptHandler => async (ctx) => {
  ...
  return { repos, worktree } satisfies PreparationOutput;
};
