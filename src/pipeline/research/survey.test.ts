...
import { makeSurvey, LEVANTAMENTO_ALLOWED_TOOLS } from "./survey.js";
...
const baseDeps = (
  makeCliProvider: ResearchDeps["makeCliProvider"]
): ResearchDeps => ({
  registry: { repos: {} },
  githubToken: "gh",
  worktreeBase: "/tmp",
...
const run = (deps: ResearchDeps, outputs: Record<string, unknown>) =>
  makeSurvey(deps)({ inputs, outputs, executionId: "e", stateId: "survey_proposal" });

describe("card-research survey", () => {
  it("runs the survey with a least-privilege allowlist that DENIES Write/Edit (criterion 2)", async () => {
    ...
    await run(deps, { preparation: prepOut });
  ...
  it("rejects a malformed survey (missing required fields)", async () => {
    ...
    await expect(run(deps, { preparation: prepOut })).rejects.toBeTruthy();
  });
