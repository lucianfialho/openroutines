...
import {
  ...
  detectFrontend,
  makeVarredura,
  type VarreduraOutput,
} from "./varredura.js";
import type { MapeamentoDeps, MapeamentoProvider } from "./index.js";
import type { CompletionResponse } from "../../provider/types.js";
import type { MapeamentoPreparacaoOutput } from "./preparacao.js";

describe("card-mapeamento varredura — change-shape mining (#162)", () => {
...
describe("card-mapeamento varredura — front-end detection (#162)", () => {
...
describe("card-mapeamento varredura — handler (#162)", () => {
  it("merges deterministic exemplars + hasFrontend into the LLM profile output", async () => {
    const workdir = mkdtempSync(join(tmpdir(), "or-varredura-"));
    writeFileSync(join(workdir, "package.json"), JSON.stringify({ dependencies: { react: "18" } }));

    const gitLog =
      `${"a".repeat(40)}\nsrc/routes/x.ts\n\n${"b".repeat(40)}\nprisma/migrations/1/migration.sql\n`;
    const provider: MapeamentoProvider = { complete: () => Effect.succeed(resp(validProfile)) };
    const deps = {
      registry: { repos: {} },
      githubToken: "gh",
      worktreeBase: "/tmp",
    ...
    const prep: MapeamentoPreparacaoOutput = {
    ...
    const out = (await makeVarredura(deps)({
      inputs: { title: "Mapear acme", description: "d" },
      outputs: { preparacao: prep },
      executionId: "e",
      stateId: "varredura",
    })) as unknown as VarreduraOutput;
