...
describe("card-mapeamento stampProfileHeader (#162)", () => {
...
const varredura = { profile: {}, coreSectionsComplete: true, goldenRoutes: ["/x"], exemplars: [{ sha: "a", changeShape: "route" }], hasFrontend: false };
const ctx = (deps: MapeamentoDeps, extraOutputs: Record<string, unknown> = {}) =>
  makePrDocs(deps)({
    inputs: { source_id: "s", task_id: "card1", title: "Mapear acme" },
    outputs: { preparacao: prep, varredura, ...extraOutputs },
    executionId: "exec1",
    stateId: "pr_docs",
  }) as Promise<PrDocsOutput>;

describe("card-mapeamento pr_docs (#162)", () => {
