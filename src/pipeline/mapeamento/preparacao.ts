/**
 * card-mapeamento / preparacao (F5 #162)
 *
 * Resolve the card's single target repo and mount a worktree on the docs
 * branch `openroutines/mapeamento-<repo>-<data>`. varredura/captura_visual
 * write docs/** into this worktree; pr_docs commits and pushes the branch.
 *
 * Unlike card-pesquisa's tolerant preparacao (a research card with no
...
import { defaultRunGit, type MapeamentoDeps } from "./index.js";

export interface MapeamentoPreparacaoRepo {
  slug: string;
  githubRepo: string;
  clonePath: string;
  baseBranch: string;
...
export interface MapeamentoPreparacaoOutput {
  repo: MapeamentoPreparacaoRepo;
  worktree: { path: string; branch: string };
  baseSha: string;
}

// RepoConfig carries no key of its own (repos.yaml maps key -> config) — recover
// the "slug" by reference, same as card-to-pr/card-pesquisa preparacao.
...
export const makePreparacao = (deps: MapeamentoDeps): ScriptHandler => async (ctx) => {
  ...
  if (!cfg) {
    ...
    return `card-mapeamento preparacao: repo não resolvível: "${repoField}"`;
  }
  ...
  return {
    ...
    baseSha: stdout.trim(),
  } satisfies MapeamentoPreparacaoOutput;
};
