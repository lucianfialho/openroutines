...
import { makeCapturaVisual, type CapturaVisualOutput } from "./captura-visual.js";
...
const ctx = (varredura: unknown) => ({
  inputs: { title: "t", description: "d", source_id: "s", task_id: "c" },
  outputs: { preparacao: prep, varredura },
  executionId: "exec1",
  stateId: "captura_visual",
});

describe("card-mapeamento captura_visual (#162)", () => {
  it("criterion 2: boots compose, captures >=1 shot per golden route + README, tears down", async () => {
    ...
    const out = (await makeCapturaVisual(deps)(ctx({ goldenRoutes }))) as unknown as CapturaVisualOutput;
  ...
  it("ALWAYS tears the compose down, even when the agent call throws", async () => {
    ...
    await expect(makeCapturaVisual(deps)(ctx({ goldenRoutes: ["/login"] }))).rejects.toThrow(/boom/);
  ...
  it("throws when deps.visual is absent (misconfiguration, not a silent skip)", async () => {
    const deps = { taskSourceFor: () => undefined } as unknown as MapeamentoDeps;
    await expect(makeCapturaVisual(deps)(ctx({ goldenRoutes: [] }))).rejects.toThrow(/not configured/);
  });
});
