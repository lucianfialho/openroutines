/**
 * card-mapeamento / validacao (F5 #162)
 *
 ...
 * A reprovado (issues.length > 0) loops ONE bounded round back to varredura
 * with the gap list.
 *
 ...
 * flakier. The issue lists validacao as "(Kimi)" and adds a third check ("cada
 ...
 * captura_visual already renders every golden route, so re-booting compose in
 * validacao would duplicate that boot for zero added coverage. Add a Kimi seam
...
export const defaultProbes: ValidacaoProbes = {
...
export const makeValidacao = (deps: MapeamentoDeps): ScriptHandler => async (ctx) => {
  ...
  const profile = varredura.profile ?? {};
  const probes = deps.validacao ?? defaultProbes;

  const issues: string[] = [];

  if (varredura.coreSectionsComplete === false) {
    issues.push("núcleo auditável incompleto: varredura marcou coreSectionsComplete=false");
  }
  ...
  return { passed: issues.length === 0, issues } satisfies ValidacaoOutput as unknown as Record<string, unknown>;
};
