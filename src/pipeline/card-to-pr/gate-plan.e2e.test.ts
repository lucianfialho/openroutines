/**
 * card-to-pr gate_plan E2E (F4 #185, D9): drives the REAL skill.yaml's
 * plan -> gate_plan -> implementation wiring through runStateMachine.
 *
 * Starts AT `plan` via the resumed-context seam (preparation's branch-
 * protection/worktree mechanics are already covered by e2e.test.ts) and stubs
 * implementation's OWN transition to `done` (verify/review/pr are also
...
/**
 * Real card-to-pr skill.yaml, with implementation's OWN transition stubbed to
 * `done` — its normal verify/review/pr continuation is already covered by
 * e2e.test.ts and is irrelevant to the plan/gate_plan wiring under test.
 */
const loadSkillStubbed = () => {
  const skill = parseSkillStateMachine(readFileSync(".gates/skills/card-to-pr/skill.yaml", "utf-8"));
  skill.states.implementation = { ...skill.states.implementation, transitions: [{ to: "done" }] };
  return skill;
};
...
const startContext = (): StateMachineContext => ({
  currentState: "plan",
  outputs: { preparation: { worktree: { path: "/tmp/or-gate-plan-e2e-fixture" } } },
});
...
const makeDeps = (): CardToPrDeps => ({
  registry: { repos: {} },
  githubToken: "gh_test",
  worktreeBase: "/tmp/or-gate-plan-e2e-worktrees",
  ledger: makeInMemoryActionLedgerRepository(),
  prLinks: makeInMemoryPrLinkRepository(),
  taskSourceFor: () => undefined,
});

describe("card-to-pr gate_plan E2E (#185)", () => {
  it("needsArchGate:false skips gate_plan entirely — zero calls to architecture-judge, straight to implementation", async () => {
    ...
    const providerRegistry: ProviderRegistry = {
      resolve: (name) => {
        ...
        throw new Error(`gate-plan e2e fixture: unexpected provider '${key}'`);
      },
    };
    ...
    expect(claudeCliCalls).toBe(2); // plan + implementation only
  });

  it("needsArchGate:true + aprovado on the first pass goes straight to implementation (never a 'spec' state)", async () => {
    const skill = loadSkillStubbed();
    expect(skill.states.spec).toBeUndefined(); // #185 sends 'aprovado' to implementation — 'spec' (D28) doesn't exist in F4
    ...
    expect(claudeCliCalls).toBe(2); // plan + implementation
    expect((repo.lastOutputs().gate_plan as { verdict: string }).verdict).toBe("aprovado");
  });

  it("refutado once (escalate:false) sends the plan back to plan, execution continues (no fail)", async () => {
    const skill = loadSkillStubbed();
    ...
    expect(claudeCliCalls).toBe(3); // plan, plan (retry), implementation
  });

  it("refutado twice on the SAME edge exhausts to blocked (never fails the execution) with outputs.gate_plan.exhausted:true, and blocked resolves blockReason 'plan-refuted-2x'", async () => {
    const skill = loadSkillStubbed();
    const planoCall = queueResponder([planoOutput(true), planoOutput(true)]);
    let claudeCliCalls = 0;
    let archJudgeCalls = 0;
    ...
    expect(r.success).toBe(true); // escaped to blocked -> done, never a hard failure
    expect(r.logs.join(" ")).toContain("Reached terminal state: done");
    expect(archJudgeCalls).toBe(2); // initial + 1 retry, then on_exhausted fires — no 3rd call
    expect(claudeCliCalls).toBe(2); // plan called before each gate_plan pass, never implementation

    const gatePlanoOut = repo.lastOutputs().gate_plan as { verdict: string; exhausted?: boolean };
    expect(gatePlanoOut.exhausted).toBe(true);
    expect(gatePlanoOut.verdict).toBe("refutado");
    const bloqueadoOut = repo.lastOutputs().blocked as { blocked: boolean; blockReason: string };
    expect(bloqueadoOut).toEqual({ blocked: true, blockReason: "plan-refuted-2x" });
  });
  ...
  it("a transition WITHOUT on_exhausted still fails outright on exhaustion (regression: F1 behavior preserved elsewhere)", async () => {
    ...
    // implementation retry edge (no on_exhausted) still fails hard when it
    // exhausts — already covered generically by state-machine-f4.test.ts;
    // this just confirms gate_plan's OWN edge is the one that gained the
    // escape, not every max_retries edge in this skill.
    const skill = parseSkillStateMachine(readFileSync(".gates/skills/card-to-pr/skill.yaml", "utf-8"));
    const revisaoToRefutacao = skill.states.review.transitions?.find((t) => t.to === "refutation");
    expect(revisaoToRefutacao?.on_exhausted).toBe("blocked"); // #153's edge already had one
    const gatePlanoToPlano = skill.states.gate_plan.transitions?.find((t) => t.to === "plan");
    expect(gatePlanoToPlano?.max_retries).toBe(1);
    expect(gatePlanoToPlano?.on_exhausted).toBe("blocked");
  });
});
