...
export const makePreparacao = (deps: CardToPrDeps): ScriptHandler => async (ctx) => {
  ...
  if (!repoConfig) {
    return { branchProtected: false, blockReason: "repo-nao-resolvivel" } satisfies PreparacaoOutput;
  }
  ...
  if (!bp.protected) {
    // No budget spent past this point — the phase ends here (routes to bloqueado).
    return { branchProtected: false, blockReason: "sem-branch-protection" } satisfies PreparacaoOutput;
  }
