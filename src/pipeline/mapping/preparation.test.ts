...
import { makePreparation, today, type MappingPreparationOutput } from "./preparation.js";
...
const run = (deps: MappingDeps, repo: string) =>
  makePreparation(deps)({ inputs: { task_id: "card1", repo }, outputs: {}, executionId: "e", stateId: "preparation" });

describe("card-mapping preparation (#162)", () => {
  it("resolves the repo and mounts a worktree on the dated mapeamento branch", async () => {
    ...
    const out = (await run(deps, "acme-widgets")) as MappingPreparationOutput;
