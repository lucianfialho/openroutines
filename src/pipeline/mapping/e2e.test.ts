/**
 * card-mapping E2E (F5 #162): drives the REAL skill.yaml through
 ...
 * mocked/injected — mirrors card-research/e2e.test.ts's harness.
 *
 * Covers the acceptance criteria: no-front-end repo skips visual_capture and
 * ships a docs-only PR; a front-end repo boots compose and captures screenshots;
 * a validation reprovado loops ONE round back to scan before the PR opens.
 */
...
import { registerMappingHandlers } from "./index.js";
...
const routine: Routine = { id: "r", triggers: [{ type: "task_source" }], pipeline: { skill: "card-mapping" } } as Routine;
...
const makeRepoStore = () => {
  ...
  store.set("exec1", { id: "exec1", routineId: "r", triggerType: "task_source", skillName: "card-mapping", status: "pending", startedAt: new Date() });
...
const makeDeps = (h: Harness, cli: (call: number) => string): MappingDeps =>
  ({
    registry,
    githubToken: "gh",
    ...
    // lets a "npm run build" command fail validation and "npm test" pass it.
    validation: {
      fileExists: () => true,
...
const drive = async (deps: MappingDeps) => {
  const skill = parseSkillStateMachine(readFileSync(".gates/skills/card-mapping/skill.yaml", "utf-8"));
  const scriptRegistry = makeScriptRegistry();
  registerMappingHandlers(scriptRegistry, deps);
...
describe("card-mapping E2E (#162)", () => {
  it("criterion 1: no front-end repo skips visual_capture and ships a docs-only PR", async () => {
    ...
    expect(h.composeUps).toBe(0); // visual_capture never ran
    expect((ctx.outputs.visual_capture as unknown)).toBeUndefined();
    ...
    const scan = ctx.outputs.scan as { exemplars: Array<{ changeShape: string }>; hasFrontend: boolean };
    expect(scan.hasFrontend).toBe(false);
    expect(new Set(scan.exemplars.map((e) => e.changeShape)).size).toBeGreaterThanOrEqual(2);
  });

  it("criterion 2: front-end repo boots compose and captures >=1 screenshot per route", async () => {
    const h: Harness = { composeUps: 0, prCreated: 0, moves: [], surveyCalls: 0 };
    // Pre-create the worktree with a front-end package.json so scan detects it.
    ...
    expect(r.success).toBe(true);
    expect(h.composeUps).toBe(1); // visual_capture ran (compose booted)
    const capture = ctx.outputs.visual_capture as { screenshots: string[]; readmeWritten: boolean };
    expect(capture.readmeWritten).toBe(true);
    expect(capture.screenshots.length).toBeGreaterThanOrEqual(1);
    expect(h.prCreated).toBe(1);
  });

  it("criterion 3: a validation reprovado loops ONE round back to scan, then the PR opens", async () => {
    const h: Harness = { composeUps: 0, prCreated: 0, moves: [], surveyCalls: 0 };
    // 1st survey lists an absent script (npm run build) -> validation reproves;
    // 2nd survey lists npm test (present) -> validation passes.
    const { r, ctx } = await drive(
      makeDeps(h, (call) => (call === 1 ? profileJson("npm run build", []) : profileJson("npm test", [])))
    );

    expect(r.success).toBe(true);
    expect(h.surveyCalls).toBe(2); // scan ran twice (original + one retry)
    const validation = ctx.outputs.validation as { passed: boolean };
    expect(validation.passed).toBe(true); // PR only opens after a pass
    expect(h.prCreated).toBe(1);
    expect(h.moves).toContainEqual({ id: "card1", state: "review" });
  });
});
