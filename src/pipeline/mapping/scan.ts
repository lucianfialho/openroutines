 ...
 *     skill.yaml uses to decide whether visual_capture runs.
 */
...
import { resolveCliProvider, SONNET_MODEL, defaultRunGit, type MappingDeps } from "./index.js";
import type { MappingPreparationOutput } from "./preparation.js";

const PROMPT_PATH = ".gates/skills/card-mapping/prompts/scan.md";
const SCHEMA_PATH = ".gates/skills/card-mapping/schemas/scan.schema.json";
const TEMPLATE_PATH = "docs/openroutines-templates/REPO-PROFILE.template.md";

/**
 * scan's Write/Edit are CONFINED to docs/** — this card never produces
...
/** A front-end framework declared in package.json deps/devDeps gates visual_capture. */
...
export const makeScan = (deps: MappingDeps): ScriptHandler => async (ctx) => {
  ...
  // Reprovado re-run: fold validation's prior gaps into the survey so the second
  // pass actually addresses them.
  const prior = ctx.outputs.validation as { passed?: boolean; issues?: string[] } | undefined;
  ...
  return { profile, coreSectionsComplete, goldenRoutes, exemplars, hasFrontend } satisfies ScanOutput as unknown as Record<string, unknown>;
};
