/**
 * card-pesquisa / levantamento_proposta (F5 #161)
 *
...
import { resolveCliProvider, SONNET_MODEL, type PesquisaDeps } from "./index.js";
import type { PreparacaoOutput } from "./preparacao.js";

const PROMPT_PATH = ".gates/skills/card-pesquisa/prompts/levantamento.md";
const SCHEMA_PATH = ".gates/skills/card-pesquisa/schemas/levantamento.schema.json";
...
export interface LevantamentoOption {
  name: string;
  tradeoffs: string;
  recommended: boolean;
}

export interface LevantamentoDoc {
  summary: string;
  currentState: string;
  options: LevantamentoOption[];
  dataChanges: string[];
  filesAffected: string[];
  phases: string[];
}

export const makeLevantamento = (deps: PesquisaDeps): ScriptHandler => async (ctx) => {
  ...
  const prior = ctx.outputs.julgamento_arquitetura as { verdict?: string; corrections?: string[] } | undefined;
