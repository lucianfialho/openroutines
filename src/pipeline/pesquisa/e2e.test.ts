/**
 * card-pesquisa E2E (F5 #161): drives the REAL skill.yaml through
 ...
 *
 * preparacao -> levantamento_proposta -> julgamento_arquitetura -> entrega ->
...
import { registerPesquisaHandlers, SONNET_MODEL } from "./index.js";
...
const routine: Routine = { id: "r", triggers: [{ type: "task_source" }], pipeline: { skill: "card-pesquisa" } } as Routine;
...
const makeRepo = () => {
  ...
  store.set("exec1", { id: "exec1", routineId: "r", triggerType: "task_source", skillName: "card-pesquisa", status: "pending", startedAt: new Date() });
...
describe("card-pesquisa E2E (#161)", () => {
  it("4 phases traverse: survey (>=2 alts) -> parecer -> milestone + 4 issues -> card comment + .md attachment -> Review (criterion 1)", async () => {
    const skill = parseSkillStateMachine(readFileSync(".gates/skills/card-pesquisa/skill.yaml", "utf-8"));
    ...
    registerPesquisaHandlers(scriptRegistry, deps);
    const repo = makeRepo();
    ...
    const lev = ctx.outputs.levantamento_proposta as { options: unknown[] };
    expect(lev.options.length).toBeGreaterThanOrEqual(2);
    const jul = ctx.outputs.julgamento_arquitetura as { verdict: string; securityOpinion: unknown };
  ...
  it("refutado loops ONE bounded round back to levantamento (with the corrections) then delivers", async () => {
    const skill = parseSkillStateMachine(readFileSync(".gates/skills/card-pesquisa/skill.yaml", "utf-8"));
    const gh: GhRec = { milestones: [], issues: [] };
    const ts: TsRec = { comments: [], attachments: [], moves: [] };
