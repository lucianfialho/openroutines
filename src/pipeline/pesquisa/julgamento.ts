/**
 * card-pesquisa / julgamento_arquitetura (F5 #161)
 *
...
import { resolveApiProvider, OPUS_MODEL, FABLE_MODEL, type PesquisaDeps } from "./index.js";

const PROMPT_PATH = ".gates/skills/card-pesquisa/prompts/julgamento.md";
const SCHEMA_PATH = ".gates/skills/card-pesquisa/schemas/julgamento.schema.json";
...
export interface JulgamentoVerdict {
...
const assertModel = (resp: CompletionResponse, expected: string): void => {
  ...
  throw new Error(
    `card-pesquisa julgamento: model mismatch — requested "${expected}" but "${resp.model}" answered; ` +
      `verdict rejected (no fallback model judges research architecture)`
  );
};

const parseVerdict = (content: string): JulgamentoVerdict => {
  ...
  return parsed as JulgamentoVerdict;
};
 ...
 */
export const runPesquisaJudgment = async (deps: PesquisaDeps, basePrompt: string): Promise<JulgamentoVerdict> => {
...
export const makeJulgamento = (deps: PesquisaDeps): ScriptHandler => async (ctx) => {
