...
4. **Triage** picks it up within ~5 minutes (5-min poller, 30-min cron as backstop, daytime) and comments the repo, type, complexity, its interpretation, acceptance criteria, and any blocking questions right on the card. Ready cards wait for the night run; cards needing input move to `Blocked` with what's missing.
